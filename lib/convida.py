...
class COnVIDa:
    ...
    def get_data_items(cls, data_items='all', regions='ES', start_date=None, end_date=None, language='ES', errors='ignore'):
        ...
        internalname_displayname_dict = cls._get_internal_names_mapping(assumed_data_type, data_items, language=language)  # get internal name - display name dict (then is used to rename again)
        ...
        for DATA_SOURCE_CLASS in requested_items_by_source.keys():
            ...
            if assumed_data_type is DataType.TEMPORAL:
                ...
                if df_data_source is not None:
                    df_data_source = cls.__complete_dates(df_data_source, start_date, end_date)    # complete with nan values those days without info  
            ...
            if df_data_source is not None:
                if df_all_data_sources is None:
                    ...
                    df_all_data_sources = pd.concat([df_all_data_sources, df_data_source], axis='columns').sort_index(axis=1)
        ...
        if assumed_data_type is DataType.TEMPORAL:
            df_all_data_sources = df_all_data_sources[(df_all_data_sources.index >= start_date) & (df_all_data_sources.index <= end_date)]  # to filter requested dates (indexes)
            df_all_data_sources = df_all_data_sources.loc[:, df_all_data_sources.columns.get_level_values('Region').isin(regions)]          # to filter dates (indexes)
        else:
            df_all_data_sources = df_all_data_sources[df_all_data_sources.index.isin(regions)] # to filter requested regions (indexes)     
            ...
            for DATA_SOURCE_CLASS in cls.__DATA_SOURCE_CLASSES:
                ...
                for data_item in DATA_SOURCE_CLASS.DATA_ITEMS:
                    if language=='internal':
                        ...
                        datasource_items_dic[DATA_SOURCE_CLASS.__name__].append(DATA_SOURCE_CLASS.DATA_ITEMS_INFO[data_item][propert][language])
        else:
            for DATA_SOURCE_CLASS in cls.__DATA_SOURCE_CLASSES:
                if DATA_SOURCE_CLASS.DATA_TYPE is data_type:
                    datasource_items_dic[DATA_SOURCE_CLASS.__name__] = []
